...
pub async fn save_history_entry(entry: &HistoryEntry) -> Result<PathBuf, String> {
    ...
    let yaml_content = serde_yaml_ng::to_string(entry)
...
pub async fn load_history_entry(path: &Path) -> Result<HistoryEntry, String> {
    ...
    let entry: HistoryEntry = serde_yaml_ng::from_str(&content)
...
mod tests {
    ...
    fn test_history_entry_serialization() {
        ...
        let yaml = serde_yaml_ng::to_string(&entry).unwrap();
        let parsed: HistoryEntry = serde_yaml_ng::from_str(&yaml).unwrap();
