...
/**
 * Build description string for toast, including duplicate count if applicable.
 */
const buildDescription = (count: number, details?: string): string | undefined => {
  if (count > 1 && details !== undefined) {
    return `(×${String(count)}) ${details}`;
  }
  if (count > 1) {
    return `This message appeared ${String(count)} times`;
  }
  return details;
};
 ...
 * Uses Sonner's native API with richColors for color-coded variants.
 *
...
export const showToast = (
  ...
  const { type, message, details, correlationId, duration } = payload;
  ...
  const description = buildDescription(count, details);
 ...
 *
 * Uses Sonner's native styling with richColors for color-coded variants:
 * - Error: Red accent
