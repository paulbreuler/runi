...
export const TooltipProvider: React.FC<{
  ...
  delayDuration?: number;
  /** Delay before closing tooltip (ms); Base UI closeDelay */
  closeDelay?: number;
}> = ({ children, delayDuration = 300, closeDelay }) => {
  return (
    <BaseUITooltip.Provider delay={delayDuration} closeDelay={closeDelay}>
      {children}
    ...
    <BaseUITooltip.Root>
      ...
      <BaseUITooltip.Portal>
        <BaseUITooltip.Positioner sideOffset={5}>
          <BaseUITooltip.Popup
            className={cn(
              'rounded-md bg-bg-elevated px-2 py-1.5 text-xs text-text-primary shadow-lg',
              'border border-border-subtle max-w-lg break-words',
