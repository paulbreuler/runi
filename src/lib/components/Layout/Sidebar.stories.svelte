...
<Story name="Expanded">
  ...
  {#snippet play({ canvasElement })}
    ...
    // Verify sidebar takes full height (check computed style)
    const sidebarStyle = window.getComputedStyle(sidebar);
    const sidebarHeight = sidebar.getBoundingClientRect().height;
    const viewportHeight = window.innerHeight;
    
    // Allow small margin for browser chrome/rounding
    expect(sidebarHeight).toBeGreaterThan(viewportHeight * 0.95);
    ...
    await expect(canvas.getByText('No history yet')).toBeInTheDocument();
  {/snippet}
</Story>
...
<Story name="Collapsed">
  ...
  {#snippet play({ canvasElement })}
    ...
    // Verify sidebar is still in DOM (but collapsed)
    await expect(sidebar).toBeInTheDocument();
    
    // Verify wrapper has width 0 (collapsed)
