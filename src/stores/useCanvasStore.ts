...
type CanvasStore = UseBoundStore<StoreApi<CanvasState>>;

const createCanvasStore = (): CanvasStore =>
  create<CanvasState>()(
    persist(
      (set, get) => ({
          ...
          if (isClosingActiveTab || shouldActivate) {
            if (activeContextId === contextId) {
              ...
              if (remainingOrder.length > 0) {
                ...
                const newIndex =
                  closedIndex < remainingOrder.length ? closedIndex : closedIndex - 1;
...
const hotMeta = import.meta as HotImportMeta;
const isHotModuleRuntime = hotMeta.hot !== undefined;

const globalCanvasStore = globalThis as typeof globalThis & {
  __RUNI_CANVAS_STORE__?: CanvasStore;
  __RUNI_CANVAS_EVENT_UNSUBSCRIBERS__?: Array<() => void>;
};

export const useCanvasStore: CanvasStore = isHotModuleRuntime
  ? (globalCanvasStore.__RUNI_CANVAS_STORE__ ??= createCanvasStore())
  : createCanvasStore();

function bindCanvasEventListeners(): Array<() => void> {
...
if (isHotModuleRuntime) {
  globalCanvasStore.__RUNI_CANVAS_EVENT_UNSUBSCRIBERS__ ??= bindCanvasEventListeners();

  hotMeta.hot?.dispose(() => {
    const unsubs = globalCanvasStore.__RUNI_CANVAS_EVENT_UNSUBSCRIBERS__;
    if (unsubs !== undefined) {
      for (const unsub of unsubs) {
        unsub();
      }
      globalCanvasStore.__RUNI_CANVAS_EVENT_UNSUBSCRIBERS__ = undefined;
    }
  });
} else {
